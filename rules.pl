exists(A,Thing) :- nextTo(A,_,Thing).
...
nextTo(A,B,Thing):-rightOf(A,B,Thing);rightOf(B,A,Thing).
...
:- exists(house(british,_,_,_,red),Houses),
    ...
    format('The zebra is owned by the ~w person~n', ZebraOwner),
    format('Water is drunk by the ~w person~n', WaterDrinker).
